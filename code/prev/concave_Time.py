...
def GetConcave():
	X1 = []
	X2 = []
	x1 = caget(ParameterList[6])
	x2 = caget(ParameterList[7])
	for i in range(10):
		X1.append(x1*(1.01+i*0.002))
		X2.append(x2*(0.985+i*0.002))
#	print X1,X2	
	f = open('tmp.dat','w')
	for i in range(10):
		caput(ParameterList[6],X1[i])
		for j in range(10):
			caput(ParameterList[7],X2[j])
			sum = 0
			for k in range(5):
		...
		f.write("\n")
	f.close()
	caput(ParameterList[6],x1)
	caput(ParameterList[7],x2)
